...
def build_part3():
    ...
    story.extend(part_divider(
        ...
        'clinical-record-content, and disclosure-accounting requirements. As of Rev. 2.14, '
    ...
    vh_rows = [
        ...
        ['2.13', 'Jul 2026',
         ...
         'Executive Director / QP'],
        ['2.14', 'Jul 2026',
         'Cover artwork restart from Rev. 2.10. Per the user\'s direction to "start with 2.10 again" with a fundamentally redesigned wellspring&nbsp;— "the heart stone has a grey stone look. A small crack in the stone has the spring rising about belly high to the tree in the back ground. there is no hole or pool"&nbsp;— Rev. 2.14 recovers the original Rev. 2.10 cover image by extracting its embedded PNG from the immutable v2.10 PDF via pdfimages (since the on-disk v2.10 source had been overwritten by subsequent v2.11/v2.12/v2.13 iterations), then performs a major in-place image edit. Three structural changes are applied: (1) the entire Rev. 2.10 wide circular stone basin, the tall narrow vertical water jet shooting up from the basin, and the circular pool of water at the base are all REMOVED and replaced with a SINGLE GREY HEART-SHAPED STONE sitting on the dry earthy ground directly in front of the tree. The heart-shaped stone is rendered in a natural cool GREY granite/river-stone color (NOT pink, NOT terracotta, NOT rose&nbsp;— a medium grey with subtle natural stony texture like weathered granite), shaped as two rounded lobes at the top curving down to meet at a gentle point at the bottom, with a mostly smooth weathered-stone surface and NO facial features, NO carvings, NO decorative markings. (2) A SMALL NATURAL CRACK runs down the surface of the grey heart-shaped stone&nbsp;— a thin, irregular, jagged geological fissure (the kind of crack an old weathered stone would naturally develop over time), NOT a smooth cut. (3) A clear water spring rises VERTICALLY FROM THE CRACK in the heart-shaped stone, reaching about BELLY-HIGH of the tree in the background (roughly the lower third of the visible tree trunk, just below where the main branches begin to spread out)&nbsp;— significantly TALLER than the heart-shaped stone itself but shorter than the tree\'s full height, and still NARROWER than the tree\'s canopy. There is NO HOLE on the ground around the stone (the water comes ONLY from the crack in the stone, not from any opening or orifice in the earth) and NO POOL of water, puddle, wet patch, or moist area on the ground at the base (the stone sits directly on the same dry warm earthy tan/orange terrain as the rest of the foreground). The fresh vivid green leaves, the stylized tree-human silhouette, the horizon line, the sunrise sky, the warm earthy color palette, the painterly style, and the 1344×768 horizontal aspect ratio are all preserved exactly from the Rev. 2.10 cover. VLM verification confirms the new image has bright green leaves, a grey heart-shaped stone (cool grey granite color) with a visible jagged crack down its surface, a clear water spring rising vertically from the crack to about belly-high of the tree (the lower third of the trunk, just below where the branches begin), no hole on the ground around the stone, no pool/puddle/wet patch on the ground at the base, the wellspring still significantly narrower than the tree\'s canopy, and zero text/letters/numbers/watermarks of any language. The standalone brand PNG (Well_Spring_Brand_Image_1344x768.png) is regenerated in lockstep. Body content (SOPs §1-§11, Protocol 22 Daily Workflow Schedules, all nine AcroForm fillable forms, §1.4(b) QP Credentialing Requirements) is unchanged from Rev. 2.13&nbsp;— only the cover image is redesigned.',
         'Executive Director / QP'],
    ]
